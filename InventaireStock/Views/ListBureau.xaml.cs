...
public partial class ListBureau : ContentPage
{
    public ListBureau()
    {
        InitializeComponent();
    ...
    protected override void OnAppearing()
    {
        ...
        if (VSITEEMPL.Bureau != "")
        {
            ...
            int nbr = inventaireDatabaseController.GetCountInventaireByBureau(VSITEEMPL.Bureau);
            if (nbr > 0)
            {
                var inv = inventaireDatabaseController.GetInventaireByBureau(VSITEEMPL.Bureau);
                inv.BUREAU = VSITEEMPL.Bureau;
    ...
    {
        InventaireDatabaseController inventaireDatabaseController = new InventaireDatabaseController();
        List<Inventaire> list = await inventaireDatabaseController.GetAllInventairesBureauDistinct();
    ...
    public void lstchanged(string keyword)
    {
        ...
        {
            InventaireDatabaseController inventaireDatabaseController = new InventaireDatabaseController();
            int nbr = inventaireDatabaseController.GetCountInventaireByBUREAUFilter(keyword);
            if (nbr > 0)
            {
                listInventaire.ItemsSource =
                 ListInv.Where(i => i.BUREAU != null && i.BUREAU.ToLower().Contains(keyword.ToLower()));
    ...
    private async void listInventaire_ItemTapped(object sender, ItemTappedEventArgs e)
    {
        ...
        {
            VSITEEMPL.Bureau = ((Inventaire)selc).BUREAU;
